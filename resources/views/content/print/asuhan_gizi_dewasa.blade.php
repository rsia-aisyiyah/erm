...
@section('content')
    ...
    <div class="border p-2" style="border: 1px solid #000; padding: 10px; margin-bottom: 10px;">
        <table width="100%" style="margin-top:0px; border-collapse: collapse; font-size:12px;">
            <tbody>
                <tr>
                    <td width="40%" style="vertical-align: top;">
                        No. Rawat : {{ $asuhanGizi['no_rawat'] }}<br>
                        Nama : {{ $pasien['no_rkm_medis'] }} - {{ $pasien['nm_pasien'] }}
                    </td>

                    <td width="25%" style="vertical-align: top;">
                        JK : {{ $pasien['jk'] == 'L' ? 'Laki-laki' : 'Perempuan' }} <br>
                        Umur : {{ $pasien['umur'] }}
                    </td>

                    <td width="35%" style="vertical-align: top;">
                        Tanggal : {{ date('d-m-Y H:i', strtotime($asuhanGizi['tanggal'])) }} <br>
                        Tgl. Lahir : {{ date('d-m-Y', strtotime($pasien['tgl_lahir'])) }}
                    </td>
                </tr>
                <tr>
                    <td colspan="3" style="padding-top: 5px;">
                        <strong>Diagnosis Awal:</strong> {{ $pasien['diagnosa_awal'] }}
                    </td>
                </tr>
            </tbody>
        </table>
