<form action="" id="formAsesmenNyeriAnak">
    ...
    <div class="alert alert-success p-2 d-none" role="alert" id="alertDischargePlanning">
        ...
        <small> <strong>Data Sudah Diisi !</strong> oleh <strong class="petugas_input"></strong> pada <strong
                class="tgl_input"></strong>, Silahkan hubungi petugas apabila terdapat ketidak sesuaian</small>
    </div>
    ...
    <div class="row gy-2 sectionMonitoring">
        <div class="col-lg-4 col-md-6 col-sm-12">
            ...
            <x-input-group class="input-group-sm">
                ...
                <x-input id="petugas" name="petugas" value="{{ session()->get('pegawai')->nama }}" class="w-25"
                    readonly />
            </x-input-group>
        </div>
        <div class="col-lg-3 col-md-6 col-sm-12">
        ...
        <div class="col-lg-4 col-md-6 col-sm-12">
            <fieldset class="border p-2">
                ...
                <x-input-group>
                    <x-checkbox-group name="pemicu" :checkboxes="[
        ...
        <div class="col-lg-4 col-md-6 col-sm-12">
            <fieldset class="border p-2">
                ...
                <x-input-group>
                    <x-checkbox-group name="kualitas" :checkboxes="[
        ...
        <div class="col-lg-4 col-md-6 col-sm-12">
            <fieldset class="border p-2">
                ...
                <x-input-group>
                    <x-checkbox-group name="lokasi" :checkboxes="[
        ...
        <div class="col-lg-6 col-md-6 col-sm-12">
            <img src="{{ asset('/img/skala_nyeri_wong_baker_color.png') }}" alt="" class="mx-auto d-block"
            ...
            <fieldset class="border p-2">
                ...
                <div class="row">
                    ...
                    <div class="col-8">
                        ...
                        <x-input-group>
                            <x-checkbox-group name="penanganan_non_farmakologi" :checkboxes="[
        ...
        'penanganan_non_farmakologiAnak5' => ['value' => 'Lainnya', 'label' => 'Lainnya'],
                            ...
                            <x-input id="penanganan_non_farmakologi_lain" name="penanganan_non_farmakologi_lain"
                                disabled class="br-full" />
                        </x-input-group>
                    </div>
                </div>
            </fieldset>
        ...
        <div class="col-lg-12 col-md-6 col-sm-12 p-3">
            <div class="row overflow-auto" id="containerAsesmenNyeri">
            </div>
...
@push('script')
    <script>
        ...
        // function renderAsesmenNyeriAnak(no_rawat) {
        ...
        //                     const action = user === objek['petugas']['nip'] ? content : '';
        //                     console.log(user, objek['petugas']);
        //                     values.push(action);
        //                 } else {
        ...
        function renderAsesmenNyeriAnak(no_rawat) {
            $.get(`${url}/asesmen-nyeri-anak`, { no_rawat: no_rawat }).done((response) => {
                const container = $('#containerAsesmenNyeri');
                container.empty();


                if (response.length === 0) {
                    container.html('<div class="col-12 text-center py-5 text-muted">Belum ada data asesmen nyeri.</div>');
                    return;
                }

                response.forEach((item, index) => {
                    console.log('index ===', index);
                    const isAwal = (index === 0);
                    const card = `
                                            <div class="col-md-4 col-lg-3">
                                                <div class="card h-100 shadow-sm">
                                                    <div class="card-header ${isAwal ? 'text-bg-primary' : 'text-bg-secondary'} ">
                                                        <h6 class="mb-0 fw-bold">${isAwal ? 'Asesmen Awal' : 'Asesmen Ulang'}</h6>
                                                        <small class="text-white">${formatTanggal(item.tanggal)}</small>
                                                    </div>
                                                    <div class="card-body">
                                                        <div class="mb-3 text-center">
                                                            ${renderSkalaNyeriUI(item.skala, item.ket_skala)}
                                                        </div>
                                                        <ul class="list-unstyled small">
                                                            <li><strong>P:</strong> ${item.pemicu || '-'}</li>
                                                            <li><strong>Q:</strong> ${item.kualitas || '-'}</li>
                                                            <li><strong>R:</strong> ${item.lokasi || '-'}</li>
                                                            <li><strong>T:</strong> ${item.waktu || '-'}</li>
                                                        </ul>
                                                        <div class="border-top pt-2 mt-2">
                                                            <small class="text-primary fw-bold">Penanganan:</small>
                                                            <p class="small mb-0">${item.penanganan_farmakologi || '-'}</p>
                                                        </div>
                                                    </div>
        ...
        // Helper untuk Skala Nyeri (Visual UX)
        function renderSkalaNyeriUI(skala, ket) {
            const images = {
                0: 'skala_1.png', 1: 'skala_1-3.png', 2: 'skala_1-3.png',
                3: 'skala_4-5.png', 4: 'skala_4-5.png', 5: 'skala_4-5.png',
                6: 'skala_6.png', 7: 'skala_6-8-9.png', 8: 'skala_6-8-9.png',
                9: 'skala_6-8-9.png', 10: 'skala_10.png'
            };
            // Sederhanakan mapping sesuai logic Anda

            const image = images[skala];
            return `<img src="${url}/public/img/${image}" width="60px" class="mb-2"><div class="h5 mb-0">${skala}</div><small class="text-muted">${ket}</small>`;
        }
        function renderActionButtons(objek) {
            // Sesuaikan dengan variabel session user yang Anda gunakan
            const loggedInNik = "{{ session()->get('pegawai')->nik }}";

            // Pastikan objek petugas ada sebelum mengakses NIP
            const petugasNip = objek.petugas ? objek.petugas.nip : null;

            // Jika user yang login adalah petugas pembuat asesmen, tampilkan tombol
            if (loggedInNik === petugasNip) {

                return `
                                                            <button type="button" class="btn btn-outline-danger btn-sm"
                                                                    onclick="deleteAsesmenNyeriAnak('${objek.no_rawat}', '${objek.tanggal}')">
                                                                <i class="bi bi-trash"></i>
                                                            </button>
                                                            <button type="button" class="btn btn-outline-primary btn-sm"
                                                                    onclick="editAsesmenNyeriAnak('${objek.no_rawat}', '${objek.tanggal}')">
                                                                <i class="bi bi-pencil"></i>
                                                            </button>
                                                        `;
            }

            // Jika bukan pembuat, kembalikan string kosong agar tidak ada tombol yang muncul
            return '';
        }

        function createAsesmenNyeriAnak() {
            ...
            $.post(`${url}/asesmen-nyeri-anak`, data).done((response) => {
                ...
                renderAsesmenNyeriAnak(data['no_rawat']);
                resetFormAsesmenNyeriAnak(data['no_rawat']);
                return response;
            }).fail((error) => {
                let errorMessage = '';
                if (error.status === 422) {
                    formAsesmenNyeriAnak.find(`input`)
                    const {
                        responseJSON
                    } = error;
