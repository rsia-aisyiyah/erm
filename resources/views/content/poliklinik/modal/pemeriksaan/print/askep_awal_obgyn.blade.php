...
@section('content')
    ...
    {{-- HEADER --}}
        ...
        <table width="100%" style="border-collapse:collapse;margin-top:0px;font-size:10px" border="1">
            ...
            <thead>
                <tr>
                    <th colspan="6" style="font-size:11px;padding:5px;text-align:center;text-transform:uppercase">
            ...
            <tbody>
                ...
                {{-- I. KEADAAN UMUM --}}
                <tr>
                    <td colspan="6" style="background:#e7e7e7;font-size:10px;font-weight:bold;text-align:center;padding:3px 5px">
                        I. KEADAAN UMUM
                    </td>
                </tr>
                ...
                <tr>
                    <td colspan="6" style="font-size:10px;padding:3px 5px">
                        <b>Keluhan Utama :</b> {{ $data->keluhan_utama ?? '-' }}
                    </td>
                </tr>
                ...
                <tr>
                    ...
                    <td colspan="5" style="font-size:10px;padding:3px 5px;vertical-align:top">
                        ...
                        &nbsp;&nbsp;
                        Usia Kehamilan : {{ $data->usia_kehamilan ?? '-' }} bln/mgg
                        &nbsp;&nbsp;
                                ...
                                <tbody>
                                    @foreach($data->riwayatPersalinan as $i => $rp)
                                        <tr>
                                            ...
                                            <td style="padding:2px 3px;text-align:center">{{ isset($rp->tgl_persalinan) ? date('Y-m-d', strtotime($rp->tgl_persalinan)) : '-' }}</td>
                        ...
                        <table width="100%" style="border-collapse:collapse;font-size:9px" border="1">
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td style="padding:2px 4px;text-align:center">{{ $data->jatuh_cara_berjalan ?? 'Tidak' }}</td>
                                    <td rowspan="2" style="padding:2px 4px;vertical-align:middle">
                                        Hasil : {{ $data->jatuh_hasil ?? 'Tidak Beresiko (Tidak Ditemukan A Dan B)' }}<br><br>
                                        Dilaporkan ke dokter ? {{ $data->jatuh_lapor_dokter ?? 'Tidak' }}
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td style="padding:2px 4px;text-align:center">{{ $data->jatuh_menopang ?? 'Tidak' }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </td>
                        ...
                        <table width="100%" style="border-collapse:collapse;font-size:9px" border="1">
                            <thead>
                                <tr style="background:#f0f0f0">
                                    ...
                                    <th style="padding:2px 4px;width:15%;text-align:center">Nilai</th>
                                </tr>
                            </thead>
                            <tbody>
                                <tr>
                                    <td style="padding:2px 4px;text-align:center">1</td>
                                    <td style="padding:2px 4px">Apakah ada penurunan berat badan yang tidak diinginkan selama enam bulan terakhir?</td>
                                    <td style="padding:2px 4px;text-align:center">
                                        {{ $data->gizi_penurunan_bb ?? 'Tidak' }} &nbsp; {{ $data->gizi_skor_1 ?? '0' }}
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td style="padding:2px 4px;text-align:center">
                                        {{ $data->gizi_nafsu_makan ?? 'Tidak' }} &nbsp; {{ $data->gizi_skor_2 ?? '0' }}
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td style="padding:2px 4px;text-align:center">{{ $data->gizi_total_skor ?? '0' }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </td>
                ...
                <tr>
                    ...
                    <td colspan="3" style="font-size:10px;padding:3px 5px;vertical-align:top">
                        <b>Nyeri Akut</b>, Penyebab : {{ $data->nyeri_penyebab ?? '-' }}<br>
                        Kualitas : {{ $data->nyeri_kualitas ?? '-' }}<br>
                        Wilayah : Lokasi {{ $data->nyeri_lokasi ?? '-' }}, Menyebar : {{ $data->nyeri_menyebar ?? 'Tidak' }}<br>
                        Severity : Skala Nyeri {{ $data->skala_nyeri ?? '-' }}, Waktu / Durasi : {{ $data->durasi_nyeri ?? '-' }}<br>
                        Nyeri hilang bila : {{ $data->nyeri_hilang ?? '-' }} - Diberitahukan pada dokter ? {{ $data->nyeri_lapor_dokter ?? 'Tidak' }}
                    </td>
                </tr>

                {{-- MASALAH & TINDAKAN --}}
                <tr>
                    <td colspan="3" style="background:#e7e7e7;font-size:10px;font-weight:bold;text-align:center;padding:3px 5px">
                        MASALAH KEBIDANAN
                    </td>
                    <td colspan="3" style="background:#e7e7e7;font-size:10px;font-weight:bold;text-align:center;padding:3px 5px">
                        TINDAKAN
